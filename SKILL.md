...
7. Identify the professional profile or combination of profiles that best matches the job. Select the CV, portfolio, projects, experience records, and links based on content and relevance—not discovery order, folder placement, or filenames alone. Tailor by elevating the strongest relevant evidence while preserving a truthful, coherent view of the candidate's wider professional range; do not reduce a multi-skilled candidate to only the keywords in the job post. If the job's track has no dedicated CV, ask the user whether to create a tailored ATS-friendly CV or use the best existing general or multi-track CV; never decide silently.
...
Read [references/cv-composition-and-emphasis.md](references/cv-composition-and-emphasis.md) before selecting CV content or declaring a tailored CV ready. It defines master-versus-targeted coverage, per-role bullet structure, evidence-safe emphasis, page-balance checks, and the separate ATS versus designed-version decision for creative roles.
