...
- Keep every skill-created file and organized candidate-professional file inside the managed `job-application-preparer/` directory at the task-workspace root. Never move, rename, overwrite, delete, or copy excluded, unrelated, unreadable, or sensitive files.
...
7. Identify the professional profile or combination of profiles that best matches the job. Select the CV, portfolio, projects, experience records, and links based on content and relevance—not discovery order or filenames alone.
...
Read [references/career-organization.md](references/career-organization.md) during workspace initialization. It defines the approved career-track layout and automatic classification process inside the managed directory.
